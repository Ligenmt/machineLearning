...
for i in range(test_num):
    ...
    y_pred = pbccrc_captcha_model.model.predict(X_verify)
    print('real: %s pred:%s  %s' % (pbccrc_captcha_model.decode(y_verify), pbccrc_captcha_model.decode(y_pred), pbccrc_captcha_model.decode(y_verify) == pbccrc_captcha_model.decode(y_pred)))
    if pbccrc_captcha_model.decode(y_verify) == pbccrc_captcha_model.decode(y_pred):
        test_acc += 1
